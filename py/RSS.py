...
class MomentumStrategy:
    ...
    def set_paths(self):
        ...
        if not self.res_dir.is_dir():
            ...
            for pnum in range(1, par[rs.qCut] + 1):
                (self.res_buyhold / str(pnum)).mkdir()
                (self.res_rebalanced / str(pnum)).mkdir()

    def form_portfos(self):
        par = self.params
        self.qBHPrts = {x: z_cf.Portfolio() for x in range(1, par[rs.qCut] + 1)}
        self.qRbPrts = {x: z_cf.Portfolio() for x in range(1, par[rs.qCut] + 1)}

    def form_returns_dfs(self):
        par = self.params
        cols = [ap.afterSkipWDDate] + [x for x in range(1, par[rs.qCut] + 1)]
        self.returnsBH = pd.DataFrame(columns = cols)
        self.returnsRb = pd.DataFrame(columns = cols)

    def load_data_from_feather(self):
        par = self.params
        fpn = z_cf.make_relavant_data_feather_pathname(
    ...
    def compute_monthly_returns(self):
        self.trade_df = z_cf.compute_returns(self.trade_df,
    ...
    def record_portfos(self):
        ...
        for key in self.qBHPrts.keys():
            ...
            pn = self.res_buyhold / f"{key}" / f'{afs}{z_ns.xlsuf}'
            # print(pn)
            # print(df)
            ...
            df = eqw_portfo_obj.portfo
            pn = self.res_rebalanced / f'{key}' / f'{afs}{z_ns.xlsuf}'
            # df = df.reset_index(drop=True)
            # df.to_feather(pn)
            df.to_excel(pn, index = False)
    ...
    def record(self):
        self.record_last_month_eqw_returns()  # self.record_portfos()
    ...
    def run(self):
        ...
        z_cf.save_df_to_xl(self.returnsBH, self.res_dir / ar.buy_and_hold)
        z_cf.save_df_to_xl(self.returnsRb, self.res_dir / ar.rebalanced)
        print('St done')
